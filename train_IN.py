...
from NetworksBlocks import SSNet_EAAE_IN, SSNet_AAEE_IN, SSNet_EAEA_IN, SSNet_AEAE_IN, SSNet_AAAA_IN, SSNet_EEEE_IN, SSNet_EEAA_IN, SSRN, SSAN, STN, CONV, SPAN, ACONV1, ACONV2
...
args.save = 'IN-train-seed{}-model-{}-arch-{}-{}-lr{}'.format(args.seed ,args.model, args.phi, time.strftime("%Y%m%d-%H%M%S"), args.learning_rate)
...
if args.model == 'SSTN':
    ...
    net = SSAN(num_classes=16, k=97)
elif args.model == 'STN':
    net = STN(num_classes=16,in_size=200)
elif args.model == 'CONV':
    net = CONV(num_classes=16,in_size=200)
elif args.model == 'ACONV1':
    net = ACONV1(num_classes=16,in_size=200)
elif args.model == 'ACONV2':
    net = ACONV2(num_classes=16,in_size=200)
elif args.model == 'SPAN':
    net = SPAN(num_classes=16,in_size=200)
else:
...
SAVE_PATH3 = args.save + '/' + str(args.model) + '_seed' + str(args.seed) + '.pth' 
...
if args.model == 'SSTN':
    if args.phi == 'EAAE':
        trained_net = SSNet_EAAE_IN()
    elif args.phi == 'AAEE':
        trained_net = SSNet_AAEE_IN()
    elif args.phi == 'AEAE':
        trained_net = SSNet_AEAE_IN()
    elif args.phi == 'EAEA':
        trained_net = SSNet_EAEA_IN()
    elif args.phi == 'EEEE':
        trained_net = SSNet_EEEE_IN()
    elif args.phi == 'AAAA':
        trained_net = SSNet_AAAA_IN()
    elif args.phi == 'EEAA':
        trained_net = SSNet_EEAA_IN()
elif args.model == 'SSRN':
    trained_net = SSRN(num_classes=16, k=97)
elif args.model == 'SSAN':
    trained_net = SSAN(num_classes=16, k=97)
elif args.model == 'STN':
    trained_net = STN(num_classes=16, in_size=200)
elif args.model == 'CONV':
    trained_net = CONV(num_classes=16, in_size=200)
elif args.model == 'ACONV1':
    trained_net = ACONV1(num_classes=16, in_size=200)
elif args.model == 'ACONV2':
    trained_net = ACONV2(num_classes=16, in_size=200)
elif args.model == 'SPAN':
    trained_net = SPAN(num_classes=16, in_size=200)
else:
